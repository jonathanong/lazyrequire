...
function lazy (require) {
  ...
  return (name) => createGetter(() => {
    try {
      ...
      console.error(chalk().yellow('Could not lazily require the module "') + chalk().red(name) + chalk().yellow('".'))
      console.error(chalk().yellow('You were expected to install this module yourself.'))
      console.error(chalk().yellow('Please run `') + chalk().cyan('npm i --save ' + name) + chalk().yellow('` in your console.'))
